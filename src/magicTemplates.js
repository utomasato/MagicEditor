    ...
    bullet : {
        ...
        converter :
            (prms, prmfgs) => {
                ...
                return `{ dict begin ~root < ~shape (empty) > spawnobj ~setMagic { ~magic $preset magicactivate $magic $root attachtoparent } def ~preset < ~main < ~duration ${prms.interval} ~startLifetime ${prms.range/prms.speed} ~startSpeed ${prms.speed} ~startSize < ~x 30 ~y 30 ~z 75 > ~startColor [ ${prms.color} 1.0 ] > ~emission < ~rateOverTime 0 ~bursts [<~count 2>] > ~colorOverLifetime < ~gradient < ~alphaKeys [ [ 0.0 0.0 ] [ 0.05 1.0 ] [ 0.95 1.0 ] [ 1.0 0.0 ] ] > > ~renderer < ~renderMode (Mesh) ~meshDistribution (NonUniformRandom) ~meshes (Bullet) ~materialName (Cross) ~alignment (Local) > > def $setMagic { $preset dup ~rotationOverLifetime < ~z 500 > put ~renderer get ~materialName (Grow_2) put } exec $setMagic { $preset ~renderer get dup ~shader (alphablended) put ~sortingFudge 10 put } exec $setMagic { $preset dup dup ~main get ~startSize < ~x 8 ~y 8 ~z 200 > put ~rotationOverLifetime < ~z 800 > put ~renderer get dup dup dup ~meshes (Cylinder) put ~materialName (Spiral) put ~shader (additive) put ~sortingFudge 0 put } exec $setMagic { $preset dup dup ~main get ~startSize 0.1 put ~trails < ~lifetime 0.2 > put ~renderer < ~materialName (Grow_1) ~trailMaterialName (Trail) ~shader (additive) > put } exec $setMagic { $preset ~renderer get dup ~shader (alphablended) put ~sortingFudge 10 put } exec $setMagic $root <${position}${rotation} ~scale ${prms.scale} > transform end}`;
            },
