    ...
    charge: {
        ...
        converter:
            (prms, prmfgs) => {
                ...
                const color1 = prms.color1.split(" ").slice(0, 3).map(tokenToString).join(" ");
                const color2 = prms.color2.split(" ").slice(0, 3).map(tokenToString).join(" ");
                const color3 = prms.color3.split(" ").slice(0, 3).map(tokenToString).join(" ");
                const color4 = prms.color4.split(" ").slice(0, 3).map(tokenToString).join(" ");
